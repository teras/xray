...
import static com.panayotis.xray.utils.EnumToIterable.toIterable;
...
public class PluginManager {
    ...
    private static void readPlugins(File path, Collection<XRayPlugin> plugins) throws IOException {
        if (path.isDirectory()) {
            ...
            for (JarEntry entry : toIterable(jar.entries()))
                if (!entry.isDirectory() && entry.getName().startsWith(PLUGIN_LOC))
