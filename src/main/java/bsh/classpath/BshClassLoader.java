 ...
 * URLClassLoader that prevents us from specifying individual classes via URLs.
 */
public class BshClassLoader extends URLClassLoader {
    ...
    /**
     * This modification allows us to reload classes which are in the Java VM
     * user classpath. We search first rather than delegate to the parent
     * classloader (or bootstrap path) first.
     *
...
// We should refactor this somehow if it sticks around
     ...
     *
     * Try designated loader if any Try our URLClassLoader paths if any Try base
     * loader if any Try system ???
     */
